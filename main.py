...
def get_contact_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="СЂСџвЂњВ± Raqamni yuborish", request_contact=True)],
            [KeyboardButton(text="СЂСџвЂќв„ў Bekor qilish")]
        ],
...
def get_main_menu(user_id):
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="рџ”— Profilni ulash", web_app=types.WebAppInfo(url=f"{WEBHOOK_URL}/?user_id={user_id}"))],
            [KeyboardButton(text="рџ’Ћ Tariflar"), KeyboardButton(text="рџ‘Ґ Do'stlarni taklif qilish")],
            [KeyboardButton(text="рџ‘¤ Mening Profilim"), KeyboardButton(text="вљ™пёЏ AI Sozlamalar")]
        ],
        resize_keyboard=True,
...
def get_tariffs_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="РІВ­С’ Standart (15 000 so'm)", callback_data="buy_standart")],
        [InlineKeyboardButton(text="СЂСџС™Р‚ SMM Pro (25 000 so'm)", callback_data="buy_smm")],
        [InlineKeyboardButton(text="СЂСџвЂќв„ў Yopish", callback_data="close_menu")]
    ])
...
def get_ai_settings_keyboard(user: dict):
    ...
    grp_status = "РІСљвЂ¦ Guruhlarda: Yoqilgan" if group_reply else "РІСњРЉ Guruhlarda: O'chirilgan"
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="СЂСџвЂњСњ Biznes Ma'lumoti (Menyu, Narxlar)", callback_data="set_biz_info")],
        [InlineKeyboardButton(text=grp_status, callback_data=grp_action)],
        [InlineKeyboardButton(text="СЂСџвЂќвЂ” Kanal/Guruh Linkini kiritish", callback_data="set_channel_link")],
        [InlineKeyboardButton(text="СЂСџвЂњСћ Mijozlarga Yangilik Tarqatish", callback_data="set_client_broadcast")],
        [InlineKeyboardButton(text="СЂСџвЂњР‰ Mening Statistikam", callback_data="show_my_stats")],
        [InlineKeyboardButton(text="СЂСџвЂќв„ў Yopish", callback_data="close_menu")]
    ])

def get_products_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="СЂСџвЂ™С– Tarif sotib olish", callback_data="btn_tariffs")],
        [InlineKeyboardButton(text="СЂСџвЂќв„ў Yopish", callback_data="close_menu")]
    ])
...
async def get_ai_response(text: str, user_id: int) -> str:
    ...
    if not has_access:
        return f"Kechirasiz, sizning 3 kunlik bepul sinov muddatingiz yoki tarifingiz tugagan! РІРЏС–\n\nIltimos, <b>СЂСџвЂ™С– Tariflar</b> bo'limidan mos tarifni xarid qiling."
...
СЂСџвЂњР‰ Holat: <i>{status_text}</i>
...
async def show_tariffs(message: types.Message):
    text = f"""СЂСџвЂ™С– <b>TARIF REJALARI VA TO'LOV</b>

РІВ­С’ <b>1. STANDART TARIF</b> РІР‚вЂќ <code>{TARIFFS['standart']['price']:,} so'm/oy</code>
{chr(10).join(['РІвЂ“Р„РїС‘РЏ ' + f for f in TARIFFS['standart']['features']])}

СЂСџС™Р‚ <b>2. SMM PRO TARIFI</b> РІР‚вЂќ <code>{TARIFFS['smm']['price']:,} so'm/oy</code>
...
СЂСџвЂ™С– <b>To'lov uchun karta:</b> <code>{CARD_NUMBER}</code>
...
async def show_products(message: types.Message):
    ...
    msg = "СЂСџвЂњВ¦ <b>BIZNING XIZMATLAR VA NARXLAR:</b>\n\n"
    for p in prods:
        msg += f"СЂСџвЂњРЉ <b>{p['name']}</b>\nСЂСџвЂ™В° Narxi: <code>{p['price']}</code>\nСЂСџвЂњСњ {p['description']}\nСЂСџвЂњвЂ№ <i>{p['details']}</i>\n\n"
...
async def show_profile(message: types.Message):
    ...
    ai_status = "РІСљвЂ¦ Yoqilgan" if user and user.get('ai_enabled') else "РІСњРЉ O'chirilgan"
...
СЂСџвЂњВ¦ Faol tarif: <b>{tariff.upper()}</b>
...
async def show_ai_settings(message: types.Message):
    ...
    if not user:
        await message.answer("РІСњРЉ Profil topilmadi. Iltimos, /start buyrug'ini bosing.")
        return
...
@dp.message(F.text == "СЂСџвЂ™С– Tariflar")
async def btn_tariffs_handler(message: types.Message):
    await show_tariffs(message)

@dp.message(F.text.in_(["СЂСџвЂТђ Do'stlarni taklif qilish", "СЂСџР‹Рѓ Bonus olish", "СЂСџвЂњВ¦ Xizmatlar"]))
...
СЂСџвЂќвЂ” Sizning taklif havolangiz:
<code>{ref_link}</code>
...
СЂСџвЂњР‰ Hozirgacha taklif qilgan do'stlaringiz: <b>{ref_count} ta</b>"""
...
async def cb_buy_standart(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"""РІВ­С’ <b>STANDART TARIF TANLANDI</b>

СЂСџвЂ™В° To'lov summasi: <b>{TARIFFS['standart']['price']:,} so'm</b>
СЂСџвЂ™С– Karta: <code>{CARD_NUMBER}</code>
СЂСџвЂВ¤ Egasi: <b>{CARD_OWNER}</b>

СЂСџвЂњС‘ <i>Iltimos, to'lovni amalga oshirib, to'lov chekining (skrinshot yoki PDF) rasmini shu chatga yuboring!</i>""", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="СЂСџвЂќв„ў Orqaga", callback_data="btn_tariffs")]]))
    await callback.answer()
...
async def cb_buy_smm(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"""СЂСџС™Р‚ <b>SMM PRO TARIFI TANLANDI</b>

СЂСџвЂ™В° To'lov summasi: <b>{TARIFFS['smm']['price']:,} so'm</b>
СЂСџвЂ™С– Karta: <code>{CARD_NUMBER}</code>
СЂСџвЂВ¤ Egasi: <b>{CARD_OWNER}</b>
...
async def cb_ai_on(callback: CallbackQuery):
    ...
    await callback.answer("РІСљвЂ¦ AI javob berish yoqildi!")
...
async def cb_ai_off(callback: CallbackQuery):
    ...
    await callback.answer("РІСњРЉ AI javob berish o'chirildi!")
...
# (Old duplicate admin handlers removed to fix conflict with correct ones below)
    ...
    if not payments:
        ...
        return
    msg = "РІРЏС– <b>KUTILAYOTGAN TO'LOVLAR:</b>\n\n"
    for p in payments:
        msg += f"СЂСџвЂ вЂќ <code>#{p['id']}</code> | ID: <code>{p['user_id']}</code>\nСЂСџвЂњВ¦ {p['tariff_type']} - <b>{p['amount']:,} so'm</b>\n"
...
async def handle_receipt(message: types.Message):
    ...
    if message.photo:
        ...
        if 'pdf' in mime or 'image' in mime:
            ...
            await message.answer("РІСњРЉ Iltimos, to'lov chekini faqat Rasm (Skrinshot) yoki PDF fayl shaklida yuboring.")
            return
    ...
    if t_key and t_key in TARIFFS:
        ...
        await message.answer(f"""СЂСџвЂњС‘ <b>To'lov cheki qabul qilindi!</b>

СЂСџвЂњВ¦ Tarif: <b>{t_data['name']}</b>
СЂСџвЂ™В° Summa: <b>{t_data['price']:,} so'm</b>
СЂСџвЂ вЂќ To'lov ID: <code>#{p_id}</code>
РІРЏС– Holat: <b>Admin tekshirmoqda. Tez orada tasdiqlanadi!</b>""")

        markup = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(text="РІСљвЂ¦ Tasdiqlash", callback_data=f"admin_approve_{p_id}"),
                    InlineKeyboardButton(text="РІСњРЉ Rad etish", callback_data=f"admin_reject_{p_id}")
                ]
            ]
        )
        caption_text = f"""СЂСџвЂ™В° <b>YANGI TO'LOV CHEKI KELDI!</b>
...
РІСљвЂ¦ Tasdiqlash: /approve_{p_id}
РІСњРЉ Rad etish: /reject_{p_id}"""
        ...
        await message.answer("СЂСџвЂњС‘ <i>Chek yuborishdan oldin iltimos menyudan tarifni tanlang:</i>", reply_markup=get_tariffs_keyboard())
...
async def cb_admin_approve(callback: CallbackQuery):
    ...
    try:
        ...
        if p and p.get('status') == 'pending':
            ...
            await bot.send_message(p['user_id'], f"СЂСџР‹вЂ° <b>TO'LOVINGIZ TASDIQLANDI!</b>\n\nСЂСџвЂњВ¦ <b>{p['tariff_type'].upper()}</b> tarifi 30 kunga faollashtirildi!\nProfilingizga AI muvaffaqiyatli ulandi.")
            try:
                await callback.message.edit_caption(caption=(callback.message.caption or "") + "\n\nРІСљвЂ¦ <b>TASDIQLANDI!</b>", reply_markup=None)
            except Exception:
                pass
            await callback.answer("РІСљвЂ¦ To'lov tasdiqlandi!")
        else:
            await callback.answer("РІСњРЉ Bu to'lov allaqachon ko'rib chiqilgan!", show_alert=True)
    except Exception as e:
...
async def cb_admin_reject(callback: CallbackQuery):
    ...
    try:
        ...
        if p:
            ...
            await bot.send_message(p['user_id'], "РІСњРЉ To'lov chekingiz tasdiqlanmadi. Iltimos, ma'lumotlarni tekshirib qayta to'lov qiling.")
            try:
                await callback.message.edit_caption(caption=(callback.message.caption or "") + "\n\nРІСњРЉ <b>RAD ETILDI!</b>", reply_markup=None)
            except Exception:
                pass
            await callback.answer("РІСњРЉ To'lov rad etildi!")
    except Exception as e:
        await callback.answer(f"Xato: {e}", show_alert=True)


# ============ PROFILNI ULASH (USERBOT SAAS) ============
@dp.message(F.text == "СЂСџвЂќвЂ” Profilni ulash (AI)")

@dp.message(F.text == "рџ”— Instagramni ulash")
async def btn_link_instagram(message: types.Message):
    ...
    markup = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="рџЊђ Instagramni UlaЕџh (Rasmiy)", url=oauth_url)]
    ])
    
    info_text = "рџ”— <b>Instagram Direct va Kommentariyalarga AIni ulash</b>\n\n" \
...
СЂСџС™Р‚ <b>2. SMM PRO TARIFI</b> РІР‚вЂќ <code>{TARIFFS['smm']['price']:,} so'm/oy</code> (25 000 so'm)
РІР‚Сћ STANDART tarifidagi barcha imkoniyatlar
РІР‚Сћ СЂСџР‹в„ў Ovozli xabarlarga ovoz bilan javob berish (Audio)
РІР‚Сћ СЂСџвЂњР‰ SMM va reklama yuborish statistikasi

<i>To'lov qilish uchun pastdagi kerakli tarif tugmasini bosing:</i>"""
        await message.answer(text, reply_markup=get_tariffs_keyboard())
        return
        
    await message.answer("СЂСџвЂњВ± <b>Telegram raqamingizni kiriting yoxud pastdagi tugmani bosing:</b>\n<i>Format: +998901234567</i>", reply_markup=get_contact_keyboard())
...
async def process_link(message: types.Message, state: FSMContext):
    ...
    await message.answer("РІСљвЂ¦ <b>Link muvaffaqiyatli saqlandi!</b>", reply_markup=get_main_menu(message.from_user.id))
    await state.clear()
...
async def cb_client_broadcast(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("СЂСџвЂњСћ <b>O'z mijozlaringizga xabar tarqatish:</b>\n\nSizning ushbu akkauntingizga oldin yozgan barcha foydalanuvchilarga avtomatik xabar (matn yoki ovozli) tarqatamiz.\n\nIltimos, tarqatmoqchi bo'lgan xabaringizni yuboring (Yoki Bekor qilishni bosing):", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="СЂСџвЂќв„ў Bekor qilish", callback_data="cancel_client_broadcast")]]))
...
async def cb_show_stats(callback: CallbackQuery):
    ...
    text = "СЂСџвЂњР‰ <b>SMM YANGILIK TARQATISH STATISTIKASI:</b>\n\n"
    total_sent = 0
    for b in broadcasts:
    ...
    await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="СЂСџвЂќв„ў Orqaga", callback_data="cancel_client_broadcast")]]))
...
async def process_client_broadcast(message: types.Message, state: FSMContext):
    ...
    await message.answer("РІРЏС– <i>Xabar barcha mijozlaringizga yuborilmoqda. Bu biroz vaqt olishi mumkin...</i>")
            ...
            for d in dialogs:
                if d.is_user and not d.entity.bot and not d.entity.is_self:
                    try:
                        ...
                        pass
            await bot.send_message(user_id, f"РІСљвЂ¦ <b>Yangilik tarqatish yakunlandi!</b>\nJami {sent_count} ta mijozingizga xabar yetkazildi.")
        except Exception as e:
            await bot.send_message(user_id, f"РІСњРЉ Tarqatishda xatolik: {e}")
...
def get_admin_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="РІРЏС– Kutilyotgan To'lovlar", callback_data="admin_payments")],
        [InlineKeyboardButton(text="СЂСџвЂњСћ Reklama Tarqatish", callback_data="admin_broadcast")],
        [InlineKeyboardButton(text="СЂСџвЂ™В° Tarif Narxlari", callback_data="admin_prices")],
        [InlineKeyboardButton(text="СЂСџвЂњР‰ Umumiy Statistika", callback_data="admin_stats")]
    ])
...
async def cb_admin_panel(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "users":
        ...
        await callback.message.edit_text(f"СЂСџвЂТђ <b>OBUNACHILAR:</b>\n\nUmumiy botdan foydalanganlar: <b>{count} ta</b>\nHozirda faol ulangan AI'lar: <b>{active} ta</b>", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="СЂСџвЂќв„ў Orqaga", callback_data="admin_back")]]))
        
    elif action == "broadcast":
        await callback.message.edit_text("СЂСџвЂњСћ <b>REKLAMA TARQATISH:</b>\n\nBotdagi barcha foydalanuvchilarga xabar tarqatish uchun xabaringizni yuboring (Rasm, Video yoki Matn):", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="СЂСџвЂќв„ў Bekor qilish", callback_data="admin_back")]]))
        ...
        if not pending:
            await callback.message.edit_text("РІСљвЂ¦ Ayni vaqtda kutilyotgan to'lovlar yo'q.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="СЂСџвЂќв„ў Orqaga", callback_data="admin_back")]]))
            return
            
        text = f"РІРЏС– <b>KUTILYOTGAN TO'LOVLAR ({len(pending)} ta):</b>\n\n"
        for p in pending:
            text += f"СЂСџвЂ™С– <b>ID:</b> #{p['id']}\nСЂСџвЂВ¤ <b>Foydalanuvchi:</b> <code>{p['user_id']}</code>\nСЂСџвЂ™В° <b>Summa:</b> {p['amount']:,} so'm ({p['tariff_type'].upper()})\n\n"
        ...
        await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="СЂСџвЂќв„ў Orqaga", callback_data="admin_back")]]))

    elif action == "prices":
        settings = await get_settings()
        sp = settings.get('standard_price', 150000)
        smm_p = settings.get('smm_price', 300000)
        await callback.message.edit_text(f"СЂСџвЂ™В° <b>TARIF NARXLARI:</b>\n\nСЂСџвЂњВ¦ Standart: <b>{sp:,} so'm</b>\nСЂСџвЂ™Р‹ SMM PRO: <b>{smm_p:,} so'm</b>\n\n<i>O'zgartirish uchun chatga yozing:</i>\n<code>/narx_standart 150000</code>\n<code>/narx_smm 300000</code>", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="СЂСџвЂќв„ў Orqaga", callback_data="admin_back")]]))
        
    elif action == "stats":
        total_payments = await db['payments'].count_documents({'status': 'approved'})
        await callback.message.edit_text(f"СЂСџвЂњР‰ <b>TIZIM STATISTIKASI:</b>\n\nMuvaffaqiyatli to'lovlar: <b>{total_payments} ta</b>\nQolgan statistika tez orada qo'shiladi.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="СЂСџвЂќв„ў Orqaga", callback_data="admin_back")]]))
...
async def process_admin_broadcast(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"РІРЏС– <i>Xabar {len(users)} ta foydalanuvchiga yuborilmoqda...</i>")
    ...
    if user_id == ADMIN_ID:
        if text.startswith('/narx_standart '):
            try:
                ...
                await message.answer(f"РІСљвЂ¦ Standart tarif narxi {price:,} so'm qilib belgilandi.")
            except:
                await message.answer("РІСњРЉ Xato format. Namuna: /narx_standart 150000")
            return
            ...
            try:
                ...
                await message.answer(f"РІСљвЂ¦ SMM PRO tarif narxi {price:,} so'm qilib belgilandi.")
            except:
                await message.answer("РІСњРЉ Xato format. Namuna: /narx_smm 300000")
            return
        ...
        if text.startswith('/approve_'):
            try:
                ...
                if p and p['status'] == 'pending':
                    ...
                    await message.answer(f"РІСљвЂ¦ <code>#{pid}</code> to'lov tasdiqlandi!")
                else:
                    await message.answer("РІСњРЉ To'lov topilmadi yoki ko'rib chiqilgan.")
            except Exception as e:
                await message.answer(f"РІСњРЉ Xato: {e}")
            return
            ...
            try:
                ...
                if p:
                    await update_payment_status(pid, 'rejected')
                    await bot.send_message(p['user_id'], "РІСњРЉ To'lov chekingiz tasdiqlanmadi. Iltimos, qayta to'lov qilib to'g'ri chekni yuboring.")
                    await message.answer(f"РІСњРЉ <code>#{pid}</code> to'lov rad etildi.")
            except Exception as e:
                await message.answer(f"РІСњРЉ Xato: {e}")
            return
            ...
            if not msg:
                await message.answer("РІСњРЉ Xabar matnini kiriting: <code>/broadcast Salom!</code>")
                return
            ...
            for u in users:
                try:
                    await bot.send_message(u['user_id'], f"СЂСџвЂњСћ <b>YANGILIK:</b>\n\n{msg}")
                    sent += 1
                    await asyncio.sleep(0.1)
                except Exception:
                    pass
            await message.answer(f"РІСљвЂ¦ Xabar {sent} ta foydalanuvchiga yuborildi.")
            return
                ...
                if t_end:
                    try:
                        ...
                        if now >= end_dt:
                            ...
                            text = (
                                "РІРЏС– <b>3 KUNLIK BEPUL SINOV MUDDATI TUGADI!</b>\n\n"
                                f"Hurmatli <b>{u.get('full_name', 'Foydalanuvchi')}</b>, profilingizga berilgan 3 kunlik sinov muddati yakuniga yetdi.\n\n"
                                "СЂСџС™Р‚ <b>AI xizmati uzilib qolmasligi uchun</b> quyidagi tugma orqali tarif xarid qiling va sun'iy intellektni faollashtiring!"
                            )
            ...
            for u in paid_users:
                ...
                if t_end:
                    try:
                        ...
                        if now >= end_dt:
                            markup = InlineKeyboardMarkup(inline_keyboard=[
                                [InlineKeyboardButton(text="СЂСџвЂ™С– Tarifni uzaytirish", callback_data="btn_tariffs")]
                            ])
                            text = (
...
async def main():
    ...
    bot_info = await bot.get_me()
    print(f"РІСљвЂ¦ BOT TAYYOR: @{bot_info.username} ({bot_info.first_name})")
    ...
    if os.environ.get("VERCEL"):
        ...
        print("СЂСџС™Р‚ TO'LIQ REJIM: Bot polling + Userbotlar birga ishlamoqda.")
